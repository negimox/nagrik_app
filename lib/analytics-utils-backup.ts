...
export async function generateReportInsights(
  ...
  try {
    ...
    console.log("Gemini Insight Prompt:", prompt);    // Call the Gemini API with the constructed prompt
    ...
    // The response structure has changed in Gemini-2.0
    if (response.candidates && response.candidates.length > 0 &&
        response.candidates[0].content?.parts?.length > 0) {
      return response.candidates[0].content.parts[0].text || "";
    }
    
    // Fallback for older API versions that might use text() method
    return typeof response.text === "function" ? response.text() : "";
  } catch (error) {
    console.error("Error generating infrastructure insights:", error);
...
export async function generateStructuredInsights(
  ...
  try {
    ...
    // Extract text from the response (handling Gemini-2.0 response structure)
    let responseText = "";
    if (response.candidates && response.candidates.length > 0 &&
        response.candidates[0].content?.parts?.length > 0) {
...
export async function generateGeospatialInsights(
  ...
  try {
    ...
    console.error("Error generating geospatial insights:", error);
...
export async function generateResourceAllocationInsights(
  ...
  try {
    ...
    console.error("Error generating resource allocation insights:", error);
